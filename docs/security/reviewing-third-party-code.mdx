...
Claude itself is great at summarizing what a script does and flagging suspicious calls. Ask it to review the code — but remember it can be fooled too ([prompt injection](/docs/security/prompt-injection) can hide in code comments), so treat its review as one input, not gospel, for anything sensitive.
